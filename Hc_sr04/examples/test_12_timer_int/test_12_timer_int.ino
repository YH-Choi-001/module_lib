...
// use the robofest HK 2022 two_stones_one_bird team's robot

#include <Hc_sr04.h>

inline int8_t setup_timer_1A_interrupt (const unsigned long us_per_interrupt, const uint16_t request_prescaler = 0);

yh::rec::Hc_sr04_timer_int uts [12] {
    // front
    yh::rec::Hc_sr04_timer_int(34, 35),
    yh::rec::Hc_sr04_timer_int(36, 37),
    yh::rec::Hc_sr04_timer_int(38, 39),
    // right
    yh::rec::Hc_sr04_timer_int(40, 41),
    yh::rec::Hc_sr04_timer_int(42, 43),
    yh::rec::Hc_sr04_timer_int(44, 45),
    // back
    yh::rec::Hc_sr04_timer_int(22, 23),
    yh::rec::Hc_sr04_timer_int(24, 25),
    yh::rec::Hc_sr04_timer_int(26, 27),
    // left
    yh::rec::Hc_sr04_timer_int(28, 29),
    yh::rec::Hc_sr04_timer_int(30, 31),
    yh::rec::Hc_sr04_timer_int(32, 33)
};
...
void setup () {
    ...
    for (uint8_t i = 0; i < 12; i++) {
        uts[i].begin();
        uts[i].set_time_interval_between_interrupts(time_interval_between_interrupt);
    }
...
ISR (TIMER1_COMPA_vect) {
    for (uint8_t i = 0; i < 12; i++) {
        uts[i].isr_individual_sensor_routine();
    }
}
...
void loop () {
    ...
    for (uint8_t i = 0; i < 12; i++) {
        Serial.print(i);
        Serial.print(": ");
        Serial.print(uts[i].read_previous_dist_cm());
        Serial.print("\t");
    }
    Serial.print('\n');
}
