    ...
    namespace rec {
        class Us_016 {
            ...
            protected:
                ...
                uint16_t raw_uts ();
                // checks is_resolution_3m and writes range_pin HIGH or LOW
                inline void refresh_range_pin ();
            public:
                ...
                uint16_t read_dist_mm (const bool refresh = true);
                // reads the distance between this ultrasound sensor and the obstacle in front of it (unit is cm)
                uint16_t read_dist_cm (const bool refresh = true);
                // set the resolution to either 1 or 3 meters
                bool set_resolution (const uint8_t resolution);
        };
    }
}
