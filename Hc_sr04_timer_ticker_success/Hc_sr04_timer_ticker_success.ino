...
const uint8_t trig_pin = 4, echo_pin = 5;

volatile uint8_t *trig_pin_out_reg;
volatile uint8_t *echo_pin_in_reg;
...
volatile unsigned long time_read;

void trig_pulse () {
  if ((*echo_pin_in_reg) & echo_pin_mask) return; // if echo pin is HIGH, just exit the function
  noInterrupts();
  (*trig_pin_out_reg) &= ~trig_pin_mask; // write trig_pin to LOW
  delayMicroseconds(2);
  (*trig_pin_out_reg) |= trig_pin_mask; // write trig_pin to HIGH
...
//  ending_time = 0;
  ...
  timer_ticks = 0;
  time_read = 0;
  while ( !((*echo_pin_in_reg) & echo_pin_mask) ) {
    // waiting the echo pin to rise from LOW to HIGH
    // __asm__ __volatile__ ("nop\n\t"); // nop - a command that takes just 1 CPU cycle
  }
  interrupts();
}
...
void setup() {
  ...
  trig_pin_out_reg = portOutputRegister(digitalPinToPort(trig_pin));
  echo_pin_in_reg = portInputRegister(digitalPinToPort(echo_pin));
...
//  if ( ( !((*echo_pin_in_reg) & echo_pin_mask) ) && (!ending_time)) { // if echo_pin is LOW && ending_time == 0
//    ending_time = micros();
//  }
//}

ISR (TIMER1_COMPA_vect) {
  if ( ( !((*echo_pin_in_reg) & echo_pin_mask) ) && (!time_read)) { // if echo_pin is LOW && time_read == 0
    time_read = timer_ticks;
  } else {
    timer_ticks++;
  }
}
...
//  if (ending_time) {
//    Serial.println((ending_time - starting_time) * 0.017);
//    trig_pulse();
//  }
  if (time_read) {
